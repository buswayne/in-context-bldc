...
def generate_c(cfg, w, T, out_path):
    ...
    a(f" *")
    a(f" * API:  void gpt_forward(const float input[W_MAX*N_U], float output[W_MAX*N_Y]);")
    ...
    a("#endif")
    a("void gpt_forward(const float input[W_MAX * N_U], float output[W_MAX * N_Y])")
    a("{")
    ...
    a("{")
    a("    static float input [W_MAX * N_U];")
    a("    static float output[W_MAX * N_Y];")
    a("    if (argc == 3) {")
    a('        FILE *fin = fopen(argv[1], "rb");')
    a("        if (!fin) { perror(argv[1]); return 1; }")
    a("        if (fread(input, sizeof(float), W_MAX*N_U, fin) != (size_t)(W_MAX*N_U)) {")
    ...
    a("        fwrite(output, sizeof(float), W_MAX*N_Y, fout);")
    a("        fclose(fout);")
    a(f'        printf("done T=%d N_U=%d N_Y=%d{NL}", W_MAX, N_U, N_Y);')
    a("    } else {")
    a("        for (int i = 0; i < W_MAX*N_U; i++) input[i] = (float)i * 0.01f;")
    ...
    a(f'        printf("W_MAX=%d  L=%d  H=%d  C=%d{NL}", W_MAX, N_LAYER, N_HEAD, N_EMBD);')
