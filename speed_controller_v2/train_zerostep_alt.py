...
checkpoint_name_to_save = "new_dataset_short_noise_scaled_h10"
checkpoint_name_to_open = "new_dataset_short_noise_scaled_h10"
...
def train(model, dataloader, criterion, optimizer, device, scaler):
    ...
    for batch in dataloader:
        ...
        batch_y_scaled = torch.mul(batch_y, scaler)
        batch_y_pred_scaled = torch.mul(batch_y_pred, scaler)

        loss = criterion(batch_y_scaled[:, :, 0], batch_y_pred_scaled[:, :, 0])
...
if __name__ == '__main__':
    ...
    scaler = torch.ones([cfg.batch_size, cfg.block_size, cfg.ny], requires_grad=False, device=device)
    for iiii in range(cfg.block_size):
        scaler[:,cfg.block_size-1-iiii,:] = forg_factor**iiii
    ...
    for epoch in range(iter_num+1, cfg.max_iters):
        ...
        train_loss = train(model, train_dl, criterion, optimizer, device, scaler)
        val_loss = validate(model, val_dl, criterion, device, scaler)
        #scheduler.step()
        # print("...")
