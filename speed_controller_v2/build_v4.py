...
def generate_c(cfg, w, T, c_path, tanh_gelu=False):
    """
    Generate GPT_model.c — the file the user copy-pastes when swapping models.
    Includes GPT_model.h for all defines and the prototype.

    tanh_gelu=False  →  exact GELU via erff() — matches training exactly.
    tanh_gelu=True   →  tanh approximation via tanhf() — use if erff() is
                         unavailable or falls back to double-precision on target.
    """
    ...
    gelu_variant = "tanh approximation" if tanh_gelu else "exact erff"

    a(f"/* AUTO-GENERATED by build_v3.py — do not edit by hand. */")
    a(f"/* Model: L={L}  H={H}  C={C}  W_MAX={T}  N_U={nu}  N_Y={ny}  bias={int(B)} */")
    a(f"/* GELU: {'tanh approximation' if tanh_gelu else 'exact erff'} */")
    a(f"/*")
    a(f" * Copy this file into Core/Src/ in your STM32CubeIDE project.")
    a(f" * GPT_model.h must be in Core/Inc/ with matching W_MAX/N_U/N_Y defines.")
    a(f" * No heap, no RTOS dependencies, no dynamic allocation.")
    a(f" */")
    a(f"")
    a(f"/* Force O3 on this translation unit regardless of project-wide setting. */")
    a(f"#pragma GCC optimize(\"O3\")")
    a(f"")
    a(f"#include \"GPT_model.h\"")
    a(f"#include <math.h>")
    a(f"#include <string.h>")
    a(f"")

    # causal mask
    mask = [1 if tk <= tq else 0 for tq in range(T) for tk in range(T)]
    a(f"/* causal mask: CAUSAL_MASK[tq*W_MAX + tk] = 1 if tk<=tq, else 0 */")
    ...
    a("/*")
    a(" * Weight matrices stored TRANSPOSED: W_T[in_dim][out_dim].")
    a(" * Enables SAXPY inner loop (sequential reads on both operands).")
    a(" * Layernorm weights are NOT transposed.")
    a(" */")
    a("")
    ...
    # linear_saxpy
    a("/* SAXPY linear: dst[i] += src[j] * W_T[j*out_dim+i]  (inner loop = SAXPY) */")
    ...
    # GELU — two variants
    if tanh_gelu:
        a("/* GELU — tanh approximation (no erff dependency, safe on all ARM targets) */")
        a("static inline float gelu(float x) {")
        a("    float x3 = x * x * x;")
        a("    return 0.5f * x * (1.0f + tanhf(0.7978845608f * (x + 0.044715f * x3)));")
        a("}")
    else:
        a("/* GELU — exact formulation via erff (matches training, available in newlib) */")
        a("/* If the linker reports undefined erff, rebuild with --tanh_gelu flag.      */")
        a("static inline float gelu(float x) {")
        a("    return 0.5f * x * (1.0f + erff(x * 0.7071067811865476f));")
        a("}")
    a("")

    # gpt_forward — no attribute needed, pragma GCC optimize at top of file covers it
    scale_str = fmt_float(float(1.0 / HS**0.5))
    a("void gpt_forward(const float *input, float *output)")
    a("{")
    a("    static float x     [W_MAX * N_EMBD];")
    a("    static float normed[W_MAX * N_EMBD];")
    a("    static float tmp   [W_MAX * N_EMBD];")
    a("    static float qkv   [W_MAX * N_EMBD3];")
    a("    static float att   [N_HEAD * W_MAX * W_MAX];")
    a("    static float vacc  [W_MAX * N_EMBD];")
    a("    static float mlph  [W_MAX * N_EMBD4];")
    a("")
    ...
    for l in range(L):
        ...
        a("        for (int tq = 0; tq < W_MAX; tq++) {")
        a("            float * __restrict__ row = att + (h * W_MAX + tq) * W_MAX;")
        a("            const float * __restrict__ q = qkv + tq*N_EMBD3 + h*N_HEAD_SIZE;")
        a("            for (int tk = 0; tk < W_MAX; tk++) {")
        a("                if (!CAUSAL_MASK[tq * W_MAX + tk]) {")
        ...
        a("")
        a("    /* softmax */")
        a("    for (int r = 0; r < N_HEAD * W_MAX; r++) {")
        a("        float * __restrict__ row = att + r * W_MAX;")
        a("        float mx = row[0];")
        a("        for (int i = 1; i < W_MAX; i++) if (row[i] > mx) mx = row[i];")
        a("        float s = 0.0f;")
        a("        for (int i = 0; i < W_MAX; i++) { row[i] = expf(row[i]-mx); s += row[i]; }")
        a("        float inv_s = 1.0f / s;")
        a("        for (int i = 0; i < W_MAX; i++) row[i] *= inv_s;")
        a("    }")
        a("")
        a("    /* weighted value accumulation */")
        a("    memset(vacc, 0, sizeof(float) * W_MAX * N_EMBD);")
        a("    for (int h = 0; h < N_HEAD; h++) {")
        a("        for (int tq = 0; tq < W_MAX; tq++) {")
        a("            float * __restrict__ dst = vacc + tq*N_EMBD + h*N_HEAD_SIZE;")
        a("            const float * __restrict__ row = att + (h*W_MAX + tq)*W_MAX;")
        a("            for (int tk = 0; tk < W_MAX; tk++) {")
    ...
    print(f"  C code generated  → {c_path}")
...
def main():
    ...
    ap.add_argument("--skip_generate", action="store_true",
                    help="Skip C generation (reuse existing GPT_model.h/.c)")
    ap.add_argument("--tanh_gelu",     action="store_true",
                    help="Use tanh GELU approximation instead of erff() "                         "(safe fallback if linker reports undefined erff)")
    ...
    bin_path = os.path.join(args.out_dir, "model.bin")
    c_path   = os.path.join(args.out_dir, "GPT_model.c")
    ext      = ".dll" if platform.system() == "Windows" else ".so"
    lib_path = os.path.join(args.out_dir, "GPT_model" + ext)
    ...
    if not args.skip_generate:
        generate_c(cfg_bin, w, T, c_path, tanh_gelu=args.tanh_gelu)
    else:
