...
lib_name = "test_l2_e32_h10.dll"
lib_name = "test_l1_e32_h10.dll"
lib_name = "test_l2_e16_h10.dll"
lib_name = "test_l1_e16_h10.dll"

# --- Step 2: load the DLL ---
lib_path = os.path.join(dll_dir, lib_name)
lib = ctypes.CDLL(lib_path)

transformer_function = lib.gpt_forward
...
#.astype(np.float32).flatten(order='F')
for i in range(10):
...
for i in range(test_length):
    ...
    x_in = x_in_big.astype(np.float32).flatten(order='F')
    transformer_function(x_in, y_out)
...
print(el_time_list_s.mean()*1e3, "ms")
print(el_time_list_s.std()*1e3, "ms")
