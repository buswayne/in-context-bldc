function [error] = Model_Id_theta_cost_function(var, input_list, output_list)
...
error = mse(y_pred(:,1), output_list(:,1)) + mse(y_pred(:,2), output_list(:,2)) + mse(sin(y_pred(:,3)), sin(output_list(:,3))) + mse(cos(y_pred(:,3)), cos(output_list(:,3)));
