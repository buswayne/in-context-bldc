...
P_list = [0.01];  %[0.01 ... 1]
I_list = [0.001, 0.01,0.1]; %[0.001 ... 10]
...
for P = P_list
    for I = I_list
        ...
        for stepsize = step_sizes
        
            reference_speed = (ones(size(time)) * stepsize)';
            reference_speed = reference_speed / 30 * pi; %in rad/s
            ...
            omega_exp = output_clean.omega;
            time_exp = output_clean.t;

            T_ass_idx = find(abs(stepsize-omega_exp)>=eps, 1, "last");
            disp(["step: ", stepsize, "Kp: ", P, "Ki: ", I])
            if isempty(T_ass_idx)
                disp("can't reach step")
            end
            T_ass = time_exp(T_ass_idx);
            if T_ass > 4
                disp("does not converge")
            else
                disp(["T_ass", T_ass])

                S_pct = max(omega_exp-stepsize)/stepsize*100;
                disp(["S_pct", S_pct])
            end
            fprintf("...\n")
            ...
            legend()
            tit = "reference step: " + stepsize + "P: " + P + "I: " + I;
            title(tit)
