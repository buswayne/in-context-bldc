 ...
 * GELULayer.c
 *
 * Code generation for function 'GELULayer'
 *
 */
 ...
 *
 */
void GELULayer_predict(const emlrtStack *sp, real32_T X_Data[640])
{
  ...
  real_T absx;
  real_T s;
  int32_T GELULayer_predict_numThreads;
  int32_T b;
  int32_T eint;
  int32_T n;
  real32_T f;
  ...
  GELULayer_predict_numThreads = emlrtAllocRegionTLSs(
      sp->tls, omp_in_parallel(), omp_get_max_threads(), omp_get_num_procs());
#pragma omp parallel num_threads(GELULayer_predict_numThreads) private(        \
        st, emlrtJBEnviron, f, z_Data, absx, s, S, R, eint, b)                 \
...
#pragma omp for nowait
    for (n = 0; n < 640; n++) {
      ...
      if (setjmp(emlrtJBEnviron) == 0) {
        ...
        /* =============================    END ================================
         */
        absx = muDoubleScalarAbs(z_Data);
        if (muDoubleScalarIsNaN(z_Data)) {
          s = z_Data;
        } else if (muDoubleScalarIsInf(z_Data)) {
          if (z_Data < 0.0F) {
            s = -1.0;
          } else {
            s = 1.0;
          ...
          if (absx < 3.7252902984619141E-9) {
            if (absx < 2.8480945388892178E-306) {
              s = 0.125 * (8.0 * z_Data + 1.0270333367641007 * z_Data);
            } else {
              s = z_Data + 0.12837916709551259 * z_Data;
            }
          } else {
            s = (real_T)z_Data * z_Data;
            s = z_Data +
                z_Data * ((s * (s * (s * (s * -2.3763016656650163E-5 -
...
/* End of code generation (GELULayer.c) */
