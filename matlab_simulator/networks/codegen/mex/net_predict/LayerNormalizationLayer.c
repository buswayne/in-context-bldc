 ...
 *
 * LayerNormalizationLayer.c
 *
 * Code generation for function 'LayerNormalizationLayer'
 *
 */
 ...
 *
 */
void LayerNormalizationLayer_predict(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
      1.01794815F, 1.03175187F, 1.02692294F,  1.03586137F,
      1.03005719F, 1.06461143F, 1.05959368F,  1.02984858F,
      1.03153837F, 1.03245747F, 1.02809381F,  1.03504574F,
      1.04863358F, 1.08597374F, 0.985173583F, 1.05767798F};
  __m128 r;
  __m128 r1;
  int32_T col;
  int32_T i;
  real32_T z_Data[160];
  real32_T zdata[160];
  real32_T accumulatedData[10];
  real32_T b_accumulatedData[10];
  for (col = 0; col < 10; col++) {
    real32_T f;
    f = sumColumnB(X_Data, col + 1) / 16.0F;
    accumulatedData[col] = f;
    i = col << 4;
    r = _mm_set1_ps(f);
    _mm_storeu_ps(&z_Data[i], _mm_sub_ps(_mm_loadu_ps(&X_Data[i]), r));
    _mm_storeu_ps(&z_Data[i + 4], _mm_sub_ps(_mm_loadu_ps(&X_Data[i + 4]), r));
    _mm_storeu_ps(&z_Data[i + 8], _mm_sub_ps(_mm_loadu_ps(&X_Data[i + 8]), r));
    _mm_storeu_ps(&z_Data[i + 12],
                  _mm_sub_ps(_mm_loadu_ps(&X_Data[i + 12]), r));
  }
  for (col = 0; col < 160; col++) {
    zdata[col] = muSingleScalarPower(z_Data[col], 2.0F);
  }
  for (col = 0; col < 10; col++) {
    b_accumulatedData[col] = sumColumnB(zdata, col + 1);
  }
  __m128 r2;
  r = _mm_loadu_ps(&b_accumulatedData[0]);
  r1 = _mm_set1_ps(16.0F);
  r2 = _mm_set1_ps(1.0E-5F);
  _mm_storeu_ps(&b_accumulatedData[0], _mm_add_ps(_mm_div_ps(r, r1), r2));
  r = _mm_loadu_ps(&b_accumulatedData[4]);
  _mm_storeu_ps(&b_accumulatedData[4], _mm_add_ps(_mm_div_ps(r, r1), r2));
  b_accumulatedData[8] = b_accumulatedData[8] / 16.0F + 1.0E-5F;
  b_accumulatedData[9] = b_accumulatedData[9] / 16.0F + 1.0E-5F;
  r = _mm_loadu_ps(&b_accumulatedData[0]);
  _mm_storeu_ps(&b_accumulatedData[0], _mm_sqrt_ps(r));
  r = _mm_loadu_ps(&b_accumulatedData[4]);
  _mm_storeu_ps(&b_accumulatedData[4], _mm_sqrt_ps(r));
  b_accumulatedData[8] = muSingleScalarSqrt(b_accumulatedData[8]);
  b_accumulatedData[9] = muSingleScalarSqrt(b_accumulatedData[9]);
  for (i = 0; i < 10; i++) {
    col = i << 4;
    r = _mm_set1_ps(accumulatedData[i]);
    r1 = _mm_set1_ps(b_accumulatedData[i]);
    _mm_storeu_ps(
        &Z_Data[col],
        _mm_mul_ps(_mm_loadu_ps(&fv[0]),
                   _mm_div_ps(_mm_sub_ps(_mm_loadu_ps(&X_Data[col]), r), r1)));
    _mm_storeu_ps(
        &Z_Data[col + 4],
        _mm_mul_ps(
            _mm_loadu_ps(&fv[4]),
            _mm_div_ps(_mm_sub_ps(_mm_loadu_ps(&X_Data[col + 4]), r), r1)));
    _mm_storeu_ps(
        &Z_Data[col + 8],
        _mm_mul_ps(
            _mm_loadu_ps(&fv[8]),
            _mm_div_ps(_mm_sub_ps(_mm_loadu_ps(&X_Data[col + 8]), r), r1)));
    _mm_storeu_ps(
        &Z_Data[col + 12],
        _mm_mul_ps(
            _mm_loadu_ps(&fv[12]),
            _mm_div_ps(_mm_sub_ps(_mm_loadu_ps(&X_Data[col + 12]), r), r1)));
  }
}
 ...
 *
 */
void b_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void c_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void d_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void e_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void f_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void g_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void h_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void i_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void j_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void k_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void l_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void m_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void n_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void o_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void p_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
 ...
 *
 */
void q_LayerNormalizationLayer_predi(const real32_T X_Data[160],
                                     real32_T Z_Data[160])
{
  static const real32_T fv[16] = {
...
/* End of code generation (LayerNormalizationLayer.c) */
