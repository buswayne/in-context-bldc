...
/*
 * Arguments    : float X_Data[640]
 * Return Type  : void
 */
void GELULayer_predict(float X_Data[640])
{
  double P;
  double R;
  double S;
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
  ...
  for (n = 0; n < 640; n++) {
    ...
    if (rtIsNaN(z_Data)) {
      ...
      if (absx < 3.7252902984619141E-9) {
        if (absx < 2.8480945388892178E-306) {
          P = 0.125 * (8.0 * z_Data + 1.0270333367641007 * z_Data);
        } else {
          P = z_Data + 0.12837916709551259 * z_Data;
        }
      } else {
        s = (double)z_Data * z_Data;
        P = z_Data + z_Data * ((s * (s * (s * (s * -2.3763016656650163E-5 -
      ...
      s = (absx - 1.0) *
          ...
          1.0;
      if (z_Data >= 0.0F) {
        P = P / s + 0.84506291151046753;
      } else {
        P = -0.84506291151046753 - P / s;
      }
    } else if (absx > 6.0) {
      if (z_Data < 0.0F) {
        P = -1.0;
      ...
      s = floor(s * 2.097152E+6) / 2.097152E+6 * P;
      P = exp(-s * s - 0.5625) * exp((s - absx) * (s + absx) + R / S) / absx;
      if (z_Data < 0.0F) {
        P--;
      } else {
        P = 1.0 - P;
    ...
    f = f / 2.0F * ((float)P + 1.0F);
    X_Data[n] = f;
  }
}

/*
 * File trailer for GELULayer.c
 *
 * [EOF]
 */
