...
for idx_exp = 1:N_exp
    ...
    while flag_control_check
        ...
        speed_input.signals.values = ones(length(time),1)*stepsize/30*pi*10;
    ...
    reference_speed = time * 0 + stepsize;
    reference_speed(time <= 1) = 0;
    ...
    T_ass_idx = find(abs(stepsize-omega)>=eps, 1, "last");
    if isempty(T_ass_idx)
        ...
        if T_ass > 4
            ...
            S_pct = max(omega-stepsize)/stepsize*100;
