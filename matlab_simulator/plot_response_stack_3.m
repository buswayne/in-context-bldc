...
models_to_test = ["test_l1_e2_h10_30k"];
...
legend([L0, L1, L2,L3], {'Reference','$S^{(1)}$', '$S^{(2)}$', '$S^{(3)}$'}, 'Location','southeast')

title(models_to_test(1), 'Interpreter','none')
figname = sprintf("exp_results_embd/stack_results%s.fig", models_to_test(1));


aaa = 0;
savefig(figname)
saveas(gcf, sprintf("exp_results_embd/stack_results%s.png", models_to_test(1)))
fig = gcf;
set(fig, 'PaperPositionMode', 'auto');
exportgraphics(fig, sprintf("exp_results_embd/stack_results%s.pdf", models_to_test(1)), 'ContentType', 'vector');
