classdef Gather_To_AddLayer1043 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        ...
        function [x906] = predict(this, onnx__MatMul_0, onnx__MatMul_0NumDims)
            if isdlarray(onnx__MatMul_0)
                onnx__MatMul_0 = stripdims(onnx__MatMul_0);
            end
            onnx__MatMul_0NumDims = numel(onnx__MatMul_0NumDims);
            ...
            [x906, x906NumDims] = Gather_To_AddFcn(onnx__MatMul_0, onnx__MatMul_0NumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {x906}))
            ...
            x906 = dlarray(single(x906), repmat('U', 1, max(2, x906NumDims)));
            if ~coder.target('MATLAB')
                x906 = extractdata(x906);
            end
        end

        function [x906] = forward(this, onnx__MatMul_0, onnx__MatMul_0NumDims)
            if isdlarray(onnx__MatMul_0)
                onnx__MatMul_0 = stripdims(onnx__MatMul_0);
            end
            onnx__MatMul_0NumDims = numel(onnx__MatMul_0NumDims);
            ...
            onnxParams.Learnables.x_h_7_ln_2_Constant_ = this.x_h_7_ln_2_Constant_;
            onnxParams.Learnables.x_ln_f_Constant_outp = this.x_ln_f_Constant_outp;
            [x906, x906NumDims] = Gather_To_AddFcn(onnx__MatMul_0, onnx__MatMul_0NumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {['as-is'], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {x906}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'Gather_To_AddLayer1043');
...
function [x906, x906NumDims, state] = Gather_To_AddFcn(onnx__MatMul_0, onnx__MatMul_0NumDims, params, varargin)
...
%
% [X906] = Gather_To_AddFcn(ONNX__MATMUL_0, PARAMS)
%			- Evaluates the imported ONNX network GATHER_TO_ADDFCN with input(s)
%			ONNX__MATMUL_0 and the imported network parameters in PARAMS. Returns
%			network output(s) in X906.
%
% [X906, STATE] = Gather_To_AddFcn(ONNX__MATMUL_0, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = Gather_To_AddFcn(ONNX__MATMUL_0, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
%				  ONNX__MATMUL_0:		[300, 10, 8]				Type: FLOAT
...
[onnx__MatMul_0, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__MatMul_0, params, varargin{:});
% Put all variables into a single struct to implement dynamic scoping:
[Vars, NumDims] = packageVariables(params, {'onnx__MatMul_0'}, {onnx__MatMul_0}, [onnx__MatMul_0NumDims]);
% Call the top-level graph function:
[x906, x906NumDims, state] = Gather_To_AddGraph1000(onnx__MatMul_0, NumDims.onnx__MatMul_0, Vars, NumDims, Training, params.State);
% Postprocess the output data
[x906] = postprocessOutput(x906, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [x906, x906NumDims1042, state] = Gather_To_AddGraph1000(onnx__MatMul_0, onnx__MatMul_0NumDims1041, Vars, NumDims, Training, state)
% Function implementing the graph 'Gather_To_AddGraph1000'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.onnx__MatMul_0 = onnx__MatMul_0;
NumDims.onnx__MatMul_0 = onnx__MatMul_0NumDims1041;

% Execute the operators:
% MatMul:
[Vars.x_wte_MatMul_output_, NumDims.x_wte_MatMul_output_] = onnxMatMul(Vars.onnx__MatMul_0, Vars.onnx__MatMul_907, NumDims.onnx__MatMul_0, NumDims.onnx__MatMul_907);
...
% Add:
Vars.x906 = Vars.lm_head_bias + Vars.x_lm_head_MatMul_out;
NumDims.x906 = max(NumDims.lm_head_bias, NumDims.x_lm_head_MatMul_out);

% Set graph output arguments from Vars and NumDims:
x906 = Vars.x906;
x906NumDims1042 = NumDims.x906;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__MatMul_0, numDataOutputs, params, varargin)
% Function to validate inputs to Gather_To_AddFcn:
p = inputParser;
isValidArrayInput = @(x)isnumeric(x) || isstring(x);
isValidONNXParameters = @(x)isa(x, 'ONNXParameters');
addRequired(p, 'onnx__MatMul_0', isValidArrayInput);
addRequired(p, 'params', isValidONNXParameters);
addParameter(p, 'InputDataPermutation', 'auto');
addParameter(p, 'OutputDataPermutation', 'auto');
addParameter(p, 'Training', false);
parse(p, onnx__MatMul_0, params, varargin{:});
...
function [onnx__MatMul_0, Training, outputDataPerms, anyDlarrayInputs] = preprocessInput(onnx__MatMul_0, params, varargin)
% Parse input arguments
[inputDataPerms, outputDataPerms, Training] = parseInputs(onnx__MatMul_0, 1, params, varargin{:});
anyDlarrayInputs = any(cellfun(@(x)isa(x, 'dlarray'), {onnx__MatMul_0}));
% Make the input variables into unlabelled dlarrays:
onnx__MatMul_0 = makeUnlabeledDlarray(onnx__MatMul_0);
% Permute inputs if requested:
onnx__MatMul_0 = permuteInputVar(onnx__MatMul_0, inputDataPerms{1}, 3);
end

function [x906] = postprocessOutput(x906, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
...
x906 = permuteOutputVar(x906, outputDataPerms{1}, 3);
end
