...
datapath = "bode_analysis_new_dataset_short_noise_h10_30k_H10H";
save_path = "bode_figures";
[~, ~] = mkdir(save_path);
delay = 7;
...
for i = 1:length(folder_list)
    ...
    % figure
    Ts = time(2)-time(1);
    data = iddata(iq_ref, error, Ts);
    g = spa(data);        
    g_etfe = etfe(data);        
    % bode(g);
    % xlim([0.4*2*pi, 10*2*pi])
...
for stepsize = stepsize_list
    ...
    xlim([0.4*2*pi, 10*2*pi])
    legend()
    suptit = sprintf("behavior around %d rpm (filtered)", stepsize);
    title(suptit)

    subplot(212)
    xlim([0.4*2*pi, 10*2*pi])

    fig_tit = sprintf("stepsize_%d_filt.png", stepsize);
...
for inertia_mult_curr = inertia_mult_list
    ...
    suptit = sprintf("behavior for inertia x %.1f (filtered)", inertia_mult_curr);
    title(suptit)

    subplot(212)
    xlim([0.4*2*pi, 10*2*pi])

    fig_tit = sprintf("inertia_%.1f_filt", inertia_mult_curr);
...
for stepsize = stepsize_list
    ...
    xlim([0.4*2*pi, 10*2*pi])
    legend()
    suptit = sprintf("behavior around %d rpm", stepsize);
    title(suptit)

    subplot(212)
    xlim([0.4*2*pi, 10*2*pi])

    fig_tit = sprintf("stepsize_%d_etfe.png", stepsize);
    saveas(gcf, fullfile(save_path, fig_tit))
...
for inertia_mult_curr = inertia_mult_list
    ...
    xlim([0.4*2*pi, 10*2*pi])
    legend('Location','southeast')
    suptit = sprintf("behavior for inertia x %.1f", inertia_mult_curr);
    title(suptit)

    subplot(212)
    xlim([0.4*2*pi, 10*2*pi])

    fig_tit = sprintf("inertia_%.1f_etfe", inertia_mult_curr);
    fig_tit = replace(fig_tit, ".", "_") + ".png";
    saveas(gcf, fullfile(save_path, fig_tit))
