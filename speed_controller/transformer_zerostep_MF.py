...
https://github.com/karpathy/nanoGPT/. flash is implemented manually to remove the tril operator cos matlab sucks
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(config.block_size):
            row = [1.0] * (i + 1) + [0.] * (config.block_size - i - 1)
        ...
        # self.register_buffer("bias", bias_no_tril(config.block_size))
    ...
    def forward(self, x):
        ...
        tmp_att = self.c_attn(x)
        # tmp_att = torch.rand(size=tmp_att.size())

        # now = time.time_ns()

        _,_,tmp_size = tmp_att.size()

        split_size = int(tmp_size/3)
        q = tmp_att[:,:,0:split_size]
        k = tmp_att[:,:,split_size:2*split_size]
        v = tmp_att[:,:,2*split_size:3*split_size]
        ...
        # now = time.time_ns()
        # q, k, v = tmp_att.split(self.n_embd, dim=2)
