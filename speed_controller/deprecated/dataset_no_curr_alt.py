...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        batch_u = torch.tensor(df[['vd', 'vq', 'omega', 'r', 'T_ass', 'S_pct']].iloc[start_idx:start_idx + self.seq_len].values,
                               dtype=torch.float32)
    ...
    def get_full_experiment(self, idx):
        ...
        batch_u = torch.tensor(df[['vd', 'vq', 'omega', 'r', 'T_ass', 'S_pct']].to_numpy(), dtype=torch.float32)
        # Add a batch dimension
        batch_y = batch_y.view(-1,1)  # Shape (1, seq_len, 1)
...
def reverse_normalization(batch_u, batch_y, batch_y_pred):
    ...
    batch_u[:, :, 0] = batch_u[:, :, 0] * (max_voltages - min_voltages) + min_voltages  # vd
    batch_u[:, :, 1] = batch_u[:, :, 1] * (max_voltages - min_voltages) + min_voltages  # vq

    # Reverse normalization for speed (omega)
    batch_u[:, :, 2] = batch_u[:, :, 2] * (max_speed - min_speed) + min_speed           # omega
    batch_u[:, :, 3] = batch_u[:, :, 3] * (max_speed - min_speed) + min_speed           # omega_ref

    # Reverse normalization for control param (T_ass, S_pct)
    batch_u[:, :, 4] = batch_u[:, :, 4] * (max_T_ass - min_T_ass) + min_T_ass           # T_ass
    batch_u[:, :, 5] = batch_u[:, :, 5] * (max_S_pct - min_S_pct) + min_S_pct           # S_pct
...
if __name__ == "__main__":
    ...
    ax2.plot(batch_u_np[:, :, 0].T, label='Batch u (vd)', color='red')
    ax2.plot(batch_u_np[:, :, 1].T, label='Batch u (vq)', color='purple')
    ax2.plot(batch_u_np[:, :, 2].T, label='Batch u (omega)', color='grey')
    ax2.plot(batch_u_np[:, :, 3].T, label='Batch u (omega_ref)')
    ax2.plot(batch_u_np[:, :, 4].T, label='Batch u (T_ass)')
    ax2.plot(batch_u_np[:, :, 5].T, label='Batch u (S_pct)')
    ax2.set_title('Batch u (vd,vq,omega,omega_ref,T_ass,S_pct)')
    ...
    for i in range(2):
        ...
        ax2 = fig.add_subplot(4,1,2, sharex = ax0)
        ax2.plot(batch_u[i,:,0],label = "$V_d$")
        ax2.plot(batch_u[i,:,1],label = "$V_q$")
        ax2.legend()
        ax3 = fig.add_subplot(4,1,3, sharex = ax0)
        ax3.plot(batch_u[i,:,2],label = "$omega$")
        ax3.plot(batch_u[i,:,3],label = "$omega_ref$")
        ax3.legend()
        ax4 = fig.add_subplot(4,2,7)
        ax4.plot(batch_u[i,:,4],label = "$T_{ass}$")
        ax4.legend()
        ax5 = fig.add_subplot(4,2,8)
        ax5.plot(batch_u[i,:,5],label = "$S_{pct}$")
        ax5.legend()
