...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attn = nn.MultiheadAttention(
            ...
            bias=config.bias,
            batch_first=True  # important: keeps (B, T, C)
        )
    ...
    def forward(self, x):
        ...
        # We pass is_causal=True to ensure causality
        B, T, C = x.size()
        device = x.device
        ...
        # diff = cols - rows
        # relu_diff = torch.nn.functional.relu(diff)
        # mask = relu_diff != 0
        # rows = np.tile(np.arange(T), (T, 1))
        # cols = np.tile(np.arange(T).reshape(T, 1), (1, T))
        
        # mask = torch.tensor(cols>rows)
        # mask = (diff.sign().clamp(min=0)).bool()

        mask_list = []
        for i in range(T):
            row = [0.0] * (i + 1) + [float('-inf')] * (T - i - 1)
            mask_list.append(row)
        mask = torch.tensor(mask_list, device=device)
        attn_output, _ = self.attn(x, x, x, is_causal=True, attn_mask=mask)
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attn = LayerNorm(config.n_embd, bias=config.bias)
