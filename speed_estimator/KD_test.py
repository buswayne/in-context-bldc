...
checkpoint_name_to_save = "model_high_speed_4layers_test_trainval_orig_corrected_val"
...
folder_path_training = ['/home/DATA/shared/st/50_percent_control/training']
folder_path_val = ['/home/DATA/shared/st/50_percent_control/validation']
...
wandb.init(
    ...
    project="in-context bldc estimator reinforce 4 layers",
    name=checkpoint_name_to_save
)

     
def train(model, dataloader, criterion, optimizer, device, reward_model):
    ...
    for batch in dataloader:
        ...
        last_predictions_rw = torch.zeros(batch_u_copy.shape[0], device=device, requires_grad=False)
        batch_y_pred_list = []  # Use a list to accumulate outputs
        batch_y_pred_list_rw = []  # Use a list to accumulate output
        ...
        for t in range(batch_u_copy.shape[1]):
            ...
            batch_u_step_rw = batch_u_copy.clone()  # Clone to avoid modification issues
            batch_u_step_rw[:, t, 4] = last_predictions_rw  # Inject last predictions
            batch_u_tmp_rw = batch_u_step_rw[:, :t+1, :]  # Take relevant time slice

            # Forward pass
            last_predictions = model(batch_u_tmp)[:, -1, :].view(-1)  # Ensure shape matches
            last_predictions_rw = reward_model(batch_u_tmp_rw)[:, -1, :].view(-1)  # Ensure shape matches

            batch_y_pred_list.append(last_predictions.unsqueeze(1))  # Store prediction
            batch_y_pred_list_rw.append(last_predictions_rw.unsqueeze(1))  # Store prediction
        ...
        batch_y_pred_rw = torch.cat(batch_y_pred_list, dim=1).unsqueeze(-1)  # Ensure shape matches batch_y
        # Compute loss
        loss = criterion(batch_y_pred_rw, batch_y_pred)
...
def validate(model, dataloader, criterion, device, reward_model):
    model.eval()
    reward_model.eval()
    running_loss = 0.0
    with torch.no_grad():
        for batch in dataloader:
            ...
            batch_y_pred_rw = torch.zeros_like(batch_y)
            ...
            last_predictions_rw = torch.zeros(batch_u_copy.shape[0], device=device)
            ...
            for t in range(batch_u_copy.shape[1]):
                ...
                batch_u_step_rw = batch_u_copy.clone()
                batch_u_step_rw[:,t,4] = last_predictions_rw
                batch_u_tmp_rw = batch_u_step_rw[:,:t+1,:]

                #update last predictions
                last_predictions = model(batch_u_tmp)[:,-1,:].view(-1)
                last_predictions_rw = reward_model(batch_u_tmp_rw)[:,-1,:].view(-1)

                batch_y_pred[:,t,0] = last_predictions
                batch_y_pred_rw[:,t,0] = last_predictions_rw
...
if __name__ == '__main__':
    ...
    #importing reward model
    out_dir = "out"
    
    reward_model_name = "model_high_speed.pt"
    out_dir = Path(out_dir)
    exp_data = torch.load(out_dir/reward_model_name, map_location=device, weights_only=False)

    model_args = exp_data["model_args"]
    gptconf = GPTConfig(**model_args)
    reward_model = GPT(gptconf).to(device)
    
    #Definizione queue
    # queue = mp.Queue(max_size = 8)

    # Avvio processo actor_proc
    # p = mp.Process(target=actor_proc, args=(queue, model, reward_model, train_dl, device))
    # p.start()
    ...
    for epoch in range(iter_num+1, cfg.max_iters):
        ...
        train_loss = train(model, train_dl, criterion, optimizer, device, reward_model)
        val_loss = validate(model, val_dl, criterion, device, reward_model)
        #scheduler.step()
        # print("...")
