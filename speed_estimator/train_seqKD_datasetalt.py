...
checkpoint_name_to_save = "model_6l_seq_03"
checkpoint_name_to_open = "model_6l_seq_03"
...
folder_training = ["simulated/50_percent_high_speed"]
folder_path_training = [os.path.join(data_path, folder) for folder in folder_training]

folder_vaildation = ["simulated/50_percent_high_speed"]
...
if alternative_batch_extractor:
    ...
    from datasetv2 import Dataset, load_dataframes_from_folder
...
if wandb_record:
    ...
    wandb.init(
        ...
        project="in-context bldc estimator 4 layers",
        name=checkpoint_name_to_save
    )


def train(model, dataloader, criterion, optimizer, device, teacher_model, kd_fact):
    '''
    ...
    for batch in dataloader:
        ...
        last_predictions_tm = torch.zeros(batch_u_copy.shape[0], device=device, requires_grad=False) # batch_u_copy.shape[0] is the batch size
        batch_y_pred_list = []  # list to accumulate outputs
        batch_y_pred_list_tm = []  # list to accumulate outputs
        ...
        for t in range(batch_u_copy.shape[1]):
            ...
            batch_u_step_tm = batch_u_copy_tm.clone()  # Clone to avoid modification issues
            batch_u_step_tm[:, t, 4] = last_predictions_tm  # Inject last predictions
            batch_u_tmp_tm = batch_u_step_tm[:, :t+1, :]  # Take relevant time slice

            # Forward pass
            last_predictions = model(batch_u_tmp)[:, -1, :].view(-1)  # Ensure shape matches
            last_predictions_tm = teacher_model(batch_u_tmp_tm)[:, -1, :].view(-1)  # Ensure shape matches

            batch_y_pred_list.append(last_predictions.unsqueeze(1))  # Store prediction
            batch_y_pred_list_tm.append(last_predictions_tm.unsqueeze(1))  # Store prediction

        # Concatenate all predictions along time dimension
        batch_y_pred = torch.cat(batch_y_pred_list, dim=1).unsqueeze(-1)  # Ensure shape matches 
        batch_y_pred_tm = torch.cat(batch_y_pred_list_tm, dim=1).unsqueeze(-1)  # Ensure shape matches batch_y

        # Compute loss
        distil_loss = criterion(batch_y_pred_tm, batch_y_pred)
        mse_loss = criterion(batch_y, batch_y_pred)

        loss = (1 - kd_fact) * mse_loss + kd_fact * distil_loss
...
def validate(model, dataloader, criterion, device, teacher_model): 
    '''
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            batch_y_pred_tm = torch.zeros_like(batch_y)
            ...
            batch_u_copy_tm = batch_u.clone().detach()
            batch_u_copy_tm[:,:,4] = 0

            # simulate step by step
            last_predictions = torch.zeros(batch_u_copy.shape[0], device=device) # batch_u_copy.shape[0] is the batch size
            last_predictions_tm = torch.zeros(batch_u_copy.shape[0], device=device) # batch_u_copy.shape[0] is the batch size
            ...
            for t in range(batch_u_copy.shape[1]):
                ...
                batch_u_step_tm = batch_u_copy_tm.clone()
                batch_u_step_tm[:,t,4] = last_predictions_tm
                batch_u_tmp_tm = batch_u_step_tm[:,:t+1,:]

                #update last predictions
                last_predictions = model(batch_u_tmp)[:,-1,:].view(-1)
                batch_y_pred[:,t,0] = last_predictions

                last_predictions_tm = teacher_model(batch_u_tmp_tm)[:,-1,:].view(-1)
                batch_y_pred_tm[:,t,0] = last_predictions_tm

            loss = criterion(batch_y_pred_tm, batch_y_pred)
...
if __name__ == '__main__':
    ...
    for path_iter in folder_path_training:
        new_dfs_train, new_dfs_val = load_dataframes_from_folder(path_iter)
        dfs_train = dfs_train + new_dfs_train
        dfs_val = dfs_val + new_dfs_val
        print(f"Loaded {len(new_dfs_train)} DataFrames from {path_iter}.")
        print(f"Loaded {len(new_dfs_val)} DataFrames from {path_iter}.")

    train_ds = Dataset(dfs=dfs_train, seq_len=cfg.seq_len)
    ...
    teacher_model_args = exp_data["model_args"]
    teacher_gptconf = GPTConfig(**teacher_model_args)
    teacher_model = GPT(teacher_gptconf).to(device)
    ...
    for epoch in range(iter_num+1, cfg.max_iters):
        ...
        train_loss = train(model, train_dl, criterion, optimizer, device, teacher_model, kd_fact)
        val_loss = validate(model, val_dl, criterion, device, teacher_model) 
        #scheduler.step()
        # print("...")
