...
checkpoint_name_to_save = "model_6l_seq"
checkpoint_name_to_open = "model_6l_seq"
...
def train(model, dataloader, criterion, optimizer, device, teacher_model, kd_fact):
    ...
    for batch in dataloader:
        ...
        last_predictions_tm = torch.zeros(batch_u_copy_tm.shape[0], device=device, requires_grad=False) # batch_u_copy.shape[0] is the batch size
...
if __name__ == '__main__':
    ...
    #Teacher model
    out_dir = "out"
    teacher_model_name = "model_high_speed.pt"
    teacher_exp_data = torch.load(out_dir/teacher_model_name, map_location=device, weights_only=False)
    teacher_model_args = teacher_exp_data["model_args"]
    gptconf = GPTConfig(**teacher_model_args)
    teacher_model = GPT(gptconf).to(device)
