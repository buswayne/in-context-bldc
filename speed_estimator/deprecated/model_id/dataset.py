...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        batch_u = torch.tensor(
            df[['ia', 'ib', 'va', 'vb', 'last_omega']].iloc[start_idx:start_idx + self.seq_len].values,
            dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    # plt.ylabel('Value')
    ax1.scatter(np.ones_like(batch_y_np[:, :, 0].T) * seq_len, batch_y_np[:, :, 0].T, label='Batch y (omega)',
                color='blue')
