...
#    mp.plot_output(t_psim, Y_psim, '.')
    ...
    freq_sim = 1e3                              # simulation frequency
    compress_factor = 3
    time = pl.arange(0.0, 20, 1./freq_sim) # create time slice vector
    ...
    for i in range(1,time.size):
        ...
        U[i-1,:] = ctl.run(500, Y[i-1,:], time[i-1])            # run the controller for the last step
